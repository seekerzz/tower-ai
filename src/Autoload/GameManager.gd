...
var secondary_mechanic: Node = null  # 次级图腾机制实例
...
func _initialize_mechanic():
	...
	# 清理旧的次级图腾机制
	if secondary_mechanic:
		secondary_mechanic.queue_free()
		secondary_mechanic = null
	...
	match core_type:
		...
		"wolf_totem": mech_script = load("res://src/Scripts/CoreMechanics/MechanicWolfTotem.gd")
	...
	# 创建次级图腾机制（如果已选择）
	if session_data and session_data.has_secondary_totem:
		_initialize_secondary_mechanic()

func _initialize_secondary_mechanic():
	"""初始化次级图腾机制实例"""
	if not session_data or not session_data.has_secondary_totem:
		return
	...
	var secondary_script = null
	match session_data.secondary_totem:
		"cow_totem": secondary_script = load("res://src/Scripts/CoreMechanics/MechanicCowTotem.gd")
		"bat_totem": secondary_script = load("res://src/Scripts/CoreMechanics/MechanicBatTotem.gd")
		"viper_totem": secondary_script = load("res://src/Scripts/CoreMechanics/MechanicViperTotem.gd")
		"butterfly_totem": secondary_script = load("res://src/Scripts/CoreMechanics/MechanicButterflyTotem.gd")
		"eagle_totem": secondary_script = load("res://src/Scripts/CoreMechanics/MechanicEagleTotem.gd")
		"wolf_totem": secondary_script = load("res://src/Scripts/CoreMechanics/MechanicWolfTotem.gd")
		_: secondary_script = load("res://src/Scripts/CoreMechanics/MechanicGeneral.gd")

	if secondary_script:
		secondary_mechanic = secondary_script.new()
		add_child(secondary_mechanic)
		print("[GameManager] 次级图腾机制已初始化: %s" % session_data.secondary_totem)
...
func _on_wave_system_started(wave_number: int, wave_type: String, difficulty: float):
	...
	# 通知当前核心机制波次开始
	if current_mechanic:
		current_mechanic.on_wave_started(wave_number, wave_type, difficulty)
...
func damage_core(amount: float, source: String = "未知来源"):
	if current_mechanic:
...
func _check_game_over():
	if session_data and session_data.core_health <= 0 and not session_data.cheat_god_mode:
		if session_data:
...
func recalculate_max_health():
	...
	if max_core_health != old_max:
		...
		if AILogger:
			...
			if AIManager:
				AIManager.broadcast_text("[核心血量计算] 波次%d | 单位数量: %d | 单位总HP: %.0f | 最大血量%s: %.0f→%.0f | 当前血量: %.0f→%.0f" % [
					wave_info, unit_count, total_unit_hp, change_type, old_max, max_core_health, old_health, core_health
				])
...
func reset_game() -> bool:
	...
	返回: 是否成功重置
	"""
	print("[Cheat] Resetting game to initial state...")

	# 检查session_data，如果未初始化则创建新的session_data
	if not session_data:
		print("[Cheat] session_data is null, initializing new session...")
		var SessionData = load("res://src/Scripts/Data/SessionData.gd")
		session_data = SessionData.new()

	print("[Cheat] Before reset - Wave: ", session_data.wave, ", Core HP: ", session_data.core_health, ", Game Over: ", session_data.game_over)

	# 调用SessionData的reset方法来完全重置所有状态
	session_data.reset()
	session_data.game_over = false

	print("[Cheat] After reset - Wave: ", session_data.wave, ", Core HP: ", session_data.core_health, ", Game Over: ", session_data.game_over)

	# 重新初始化网格，这将重置spawn_tiles数组
	if grid_manager:
		grid_manager.create_initial_grid()

	# 清空所有单位
	if grid_manager:
		# 清除所有放置的单位 (通过tiles字典遍历)
		for key in grid_manager.tiles.keys():
			var tile = grid_manager.tiles[key]
			if tile and tile.unit:
				...
				tile.unit = null
			# 清除occupied_by标记
			if tile and tile.occupied_by != Vector2i.ZERO:
				tile.occupied_by = Vector2i.ZERO
	...
	# 重置波次系统
	if wave_system_manager:
		wave_system_manager.current_wave = 1
		wave_system_manager.is_wave_active = false

	# 重置AIManager的is_game_over标志
	if AIManager:
		AIManager.is_game_over = false
		print("[Cheat] AIManager is_game_over reset to false")

	# 通知游戏重置
	resource_changed.emit()
	game_reset.emit()
	print("[Cheat] Game reset complete. Wave: 1, Core HP: ", max_core_health)

	return true

func spawn_unit(unit_id: String, grid_pos: Vector2i = Vector2i(-1, -1), unit_level: int = 1) -> bool:
	"""
	在指定位置生成单位

	参数:
		unit_id: 单位ID（如 "iron_turtle", "hedgehog"）
		grid_pos: 网格位置，如果为(-1, -1)则自动寻找空位
		unit_level: 单位等级（1-3），默认为1

	返回: 是否成功生成
	"""
	# 验证单位ID
	if not Constants.UNIT_TYPES.has(unit_id):
		print("[Cheat] Error: Unknown unit_id '", unit_id, "'")
		return false

	if not grid_manager:
		print("[Cheat] Error: GridManager not available")
		return false

	# 如果没有指定位置，寻找第一个空位
	var target_pos = grid_pos
	if target_pos == Vector2i(-1, -1):
		target_pos = _find_empty_tile()
		if target_pos == Vector2i(-1, -1):
			print("[Cheat] Error: No empty tile available")
			return false

	# 验证位置是否有效
	var tile_key = grid_manager.get_tile_key(target_pos.x, target_pos.y)
	print("[Cheat] Debug: tile_key = ", tile_key, ", target_pos = ", target_pos)
	if not grid_manager.tiles.has(tile_key):
		print("[Cheat] Error: Invalid tile position ", target_pos, ", tile_key = ", tile_key)
		return false

	# 检查目标位置是否已被占用 (检查unit和occupied_by)
	var target_tile = grid_manager.tiles[tile_key]
	print("[Cheat] Debug: target_tile.unit = ", target_tile.unit, ", occupied_by = ", target_tile.occupied_by)
	if target_tile.unit != null or target_tile.occupied_by != Vector2i.ZERO:
		print("[Cheat] Error: Tile already occupied at ", target_pos)
		return false

	# 生成单位
	var unit_scene = preload("res://src/Scenes/Game/Unit.tscn")
	var unit = unit_scene.instantiate()

	# 先设置等级再setup，确保行为能读取正确等级
	if unit_level > 1 and unit_level <= 3:
		unit.level = unit_level
		print("[Cheat] Pre-set unit level to ", unit_level)
	...
	# 设置起始位置（用于战斗逻辑）
	unit.start_position = unit.global_position

	# 确保单位被正确初始化到场景树
	# 注意：在Godot 4中，需要确保节点在场景树中才能正确处理
	# FIX-CRITICAL-001: 确保单位在场景树中并启用处理
	if not unit.is_inside_tree():
		print("[Cheat] Warning: Unit not in tree after add_child, retrying...")
		# 强制添加到场景树
		await get_tree().process_frame
		if not unit.is_inside_tree():
			print("[Cheat] Error: Unit still not in tree after await!")
			return false

	# 设置process_mode确保单位能正常处理
	unit.process_mode = Node.PROCESS_MODE_INHERIT
	unit.set_physics_process(true)
	unit.set_process(true)
	print("[Cheat] Unit process enabled, in tree: ", unit.is_inside_tree(), ", process_mode: ", unit.process_mode)

	# 注册单位到瓦片系统（支持多格单位）
	var w = unit.unit_data.size.x
	var h = unit.unit_data.size.y
	grid_manager._set_tiles_occupied(target_pos.x, target_pos.y, w, h, unit)

	# 重新计算buff和最大生命值
	grid_manager.recalculate_buffs()
	recalculate_max_health()

	print("[Cheat] Spawned unit '", unit_id, "' at position ", target_pos, " level: ", unit.level, " size: ", w, "x", h, " start_pos: ", unit.start_position)
...
func _find_empty_tile() -> Vector2i:
	...
	for x in range(5):
		for y in range(5):
			...
			var tile_key = grid_manager.get_tile_key(x, y)
			if grid_manager.tiles.has(tile_key):
				var tile = grid_manager.tiles[tile_key]
				if tile and tile.unit == null and tile.occupied_by == Vector2i.ZERO:
					return pos
