...
var core_health: float = 500.0:
	set(value):
		var old_value = core_health
		core_health = clamp(value, 0, max_core_health)
		if old_value != core_health:
			# 记录核心血量变化日志
			if AILogger:
...
# ===== 次级图腾（击败第1个Boss后选择） =====
var secondary_totem: String = ""  # 击败第1个Boss后可选择的次级图腾ID
var has_secondary_totem: bool:  # 是否已选择次级图腾
	get:
		return secondary_totem != ""

# ===== 第三图腾（击败第2个Boss后选择） =====
var third_totem: String = ""  # 击败第2个Boss（第12波）后可选择的第三图腾ID
var has_third_totem: bool:  # 是否已选择第三图腾
	get:
		return third_totem != ""
