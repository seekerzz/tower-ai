...
func on_damage_taken(amount: float, source: Node) -> float:
	...
	if AILogger and amount > 0:
		...
		AILogger.event("[UNIT_BUFF] 铁甲龟 硬化皮肤生效 | 减伤: %d | 原始伤害: %.0f | 实际伤害: %.0f | 来源: %s" % [damage_reduction, amount, reduced, source_name])
	...
	if unit.level >= 3 and reduced <= 0 and amount > 0:
		...
		if AILogger:
			AILogger.event("[UNIT_HEAL] 铁甲龟 绝对防御回血 | 回复: %.0f HP (核心) | Lv.3技能触发" % heal_amount)
			if AIManager:
