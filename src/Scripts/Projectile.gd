...
func _process(delta):
	...
	if !is_instance_valid(target) and stats.has("target_pos") and source_unit and source_unit.is_in_group("enemies"):
		if global_position.distance_to(stats.target_pos) < 15.0:
			var enemy_name = source_unit.type_key if source_unit.has_method("type_key") else "远程敌人"
			GameManager.damage_core(damage, enemy_name + "远程攻击")
...
func _handle_hit(target_node):
	...
	if target_node.is_in_group("enemies"):
		...
		if is_critical:
			...
			# 调用次级图腾的暴击回调（如鹰图腾的暴击回响）
			if GameManager.secondary_mechanic and GameManager.secondary_mechanic.has_method("on_projectile_crit"):
				GameManager.secondary_mechanic.on_projectile_crit(self, target_node)
			# 记录暴击触发日志
			if AILogger:
