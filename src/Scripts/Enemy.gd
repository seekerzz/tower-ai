...
var _bleed_accumulated_damage: float = 0.0  # 累积伤害用于日志输出
...
func apply_debuff(type: String, stacks: int = 1):
	match type:
		...
		"vulnerable":
			apply_status(load("res://src/Scripts/Effects/VulnerableEffect.gd"), {"duration": 5.0, "stacks": stacks})
...
func _process_bleed_damage(delta: float):
	if bleed_stacks > 0:
		...
		# Accumulate damage for logging (to show meaningful numbers)
		_bleed_accumulated_damage += damage
		...
		# Log accumulated damage periodically (only if meaningful damage accumulated)
		if should_show_text and _bleed_accumulated_damage >= 0.1:
...
func die(killer_unit = null):
	...
	if !handled:
		# FIX: CRASH-COW-TOTEM-MASS-DEATH
		# Use call_deferred to avoid "Can't change this state while flushing queries" error
		# when multiple enemies die simultaneously from global damage (cow totem counter-attack)
		call_deferred("_deferred_queue_free")

func _deferred_queue_free():
	"""Deferred cleanup to prevent physics state conflicts during mass enemy deaths"""
	queue_free()
