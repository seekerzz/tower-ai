...
func refresh_shop() -> Dictionary:
	...
	var secondary_faction = ""
	var third_faction = ""
	if GameManager.session_data:
		secondary_faction = GameManager.session_data.secondary_totem
		third_faction = GameManager.session_data.third_totem
	print("[BoardController] refresh_shop - player_faction: ", player_faction, " secondary_faction: ", secondary_faction, " third_faction: ", third_faction)

	# DEBUG: 检查 core_type 是否为空
	if player_faction == "":
		print("[BoardController] WARNING: player_faction is empty! GameManager.core_type = '", GameManager.core_type, "'")
		if AIManager:
			AIManager.broadcast_text("[系统警告] 商店刷新时阵营未设置，使用默认单位")

	# 分离主阵营单位、次级阵营单位、第三阵营单位和通用单位
	var main_faction_units = []
	var secondary_faction_units = []
	var third_faction_units = []
	var universal_units = []
	for unit_key in Constants.UNIT_TYPES.keys():
		...
		if unit_faction == player_faction:
			main_faction_units.append(unit_key)
			print("[BoardController] Found main faction unit: ", unit_key, " (", unit_faction, ")")
		elif unit_faction == secondary_faction and secondary_faction != "":
			secondary_faction_units.append(unit_key)
			print("[BoardController] Found secondary faction unit: ", unit_key, " (", unit_faction, ")")
		elif unit_faction == third_faction and third_faction != "":
			third_faction_units.append(unit_key)
			print("[BoardController] Found third faction unit: ", unit_key, " (", unit_faction, ")")
		elif unit_faction == "universal":
			universal_units.append(unit_key)

	print("[BoardController] refresh_shop - main_faction_units: ", main_faction_units.size(),
		" secondary_faction_units: ", secondary_faction_units.size(),
		" third_faction_units: ", third_faction_units.size(),
		" universal_units: ", universal_units.size())

	# DEBUG: 广播阵营单位信息
	if AIManager and (main_faction_units.size() > 0 or secondary_faction_units.size() > 0 or third_faction_units.size() > 0):
		var faction_info = "[商店调试] 主阵营: " + player_faction + "(" + str(main_faction_units.size()) + ")"
		if secondary_faction != "":
			faction_info += " 次级阵营: " + secondary_faction + "(" + str(secondary_faction_units.size()) + ")"
		if third_faction != "":
			faction_info += " 第三阵营: " + third_faction + "(" + str(third_faction_units.size()) + ")"
		faction_info += " 通用: " + str(universal_units.size())
		AIManager.broadcast_text(faction_info)
	...
	for i in range(4):
		if session_data.is_shop_slot_locked(i):
			...
			# 单位池混合规则：
			# - 如果有第三图腾：主40%、次级25%、第三20%、通用15%
			# - 如果有次级图腾：主40%、次级30%、通用30%
			# - 如果只有主图腾：主70%、通用30%
			var roll = randf()
			if third_faction != "" and third_faction_units.size() > 0:
				# 三阵营模式：40%主 + 25%次 + 20%第三 + 15%通用
				if main_faction_units.size() > 0 and roll < 0.4:
					new_shop[i] = main_faction_units.pick_random()
				elif secondary_faction_units.size() > 0 and roll < 0.65:
					new_shop[i] = secondary_faction_units.pick_random()
				elif third_faction_units.size() > 0 and roll < 0.85:
					new_shop[i] = third_faction_units.pick_random()
				elif universal_units.size() > 0:
					new_shop[i] = universal_units.pick_random()
				elif main_faction_units.size() > 0:
					new_shop[i] = main_faction_units.pick_random()
				elif secondary_faction_units.size() > 0:
					new_shop[i] = secondary_faction_units.pick_random()
				else:
					new_shop[i] = third_faction_units.pick_random()
			elif secondary_faction != "" and secondary_faction_units.size() > 0:
				# 双阵营模式：40%主 + 30%次 + 30%通用
				if main_faction_units.size() > 0 and roll < 0.4:
					new_shop[i] = main_faction_units.pick_random()
				elif secondary_faction_units.size() > 0 and roll < 0.7:
					new_shop[i] = secondary_faction_units.pick_random()
				elif universal_units.size() > 0:
					new_shop[i] = universal_units.pick_random()
				elif main_faction_units.size() > 0:
				...
				# 单阵营模式：70%主 + 30%通用
				if main_faction_units.size() > 0 and roll < 0.7:
					new_shop[i] = main_faction_units.pick_random()
				elif universal_units.size() > 0:
					new_shop[i] = universal_units.pick_random()
				elif main_faction_units.size() > 0:
...
func _get_units_for_faction(faction: String) -> Array:
	var result = []
	print("[BoardController] _get_units_for_faction - looking for faction: ", faction)
	for unit_key in Constants.UNIT_TYPES.keys():
		var unit_data = Constants.UNIT_TYPES[unit_key]
		var unit_faction = unit_data.get("faction", "universal")
		if unit_faction == faction or unit_faction == "universal":
			...
			if unit_faction == faction:
				print("[BoardController] _get_units_for_faction - found faction unit: ", unit_key, " faction: ", unit_faction)
	print("[BoardController] _get_units_for_faction - total result count: ", result.size())
	if result.is_empty():
...
func start_wave() -> Dictionary:
	...
	@return: 结果字典 { "success": bool, "error_message": String }
	"""
	print("[BoardController] start_wave() called")

	if session_data == null:
		var msg = "SessionData not initialized"
		print("[BoardController] Error: ", msg)
	...
	print("[BoardController] SessionData.is_wave_active set to true")

	# 委托给 GameManager 的波次系统
	if GameManager.wave_system_manager:
		print("[BoardController] Calling GameManager.wave_system_manager.start_wave() with wave number: ", session_data.wave)
		GameManager.wave_system_manager.start_wave(session_data.wave)
	else:
		print("[BoardController] Error: GameManager.wave_system_manager is null")
		session_data.is_wave_active = false
		return {"success": false, "error_message": "WaveSystemManager not initialized"}

	print("[BoardController] start_wave() completed successfully")
	return {"success": true}
