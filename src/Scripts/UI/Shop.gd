...
func _ready():
	print("[Shop] _ready - started, core_type: ", GameManager.core_type)
	...
	if GameManager.core_type == "":
		print("[Shop] _ready - waiting for core_type_changed...")
		await GameManager.core_type_changed
		print("[Shop] _ready - received core_type_changed, core_type: ", GameManager.core_type)

	# 执行商店刷新（使用 faction 过滤）
	print("[Shop] _ready - calling _perform_refresh")
	_perform_refresh()
	update_ui()
...
func update_ui():
	...
	# 更新所有商店卡片的价格可负担状态
	_update_shop_cards_affordability()
	# update_wave_info() # REMOVED

func _update_shop_cards_affordability():
	"""更新所有商店卡片的价格可负担状态"""
...
func _perform_refresh():
	...
	print("[Shop] _perform_refresh - player_faction: ", player_faction)
	...
	for unit_key in Constants.UNIT_TYPES.keys():
		...
		if unit_faction == player_faction:
			...
			print("[Shop] _perform_refresh - found faction unit: ", unit_key, " faction: ", unit_faction)
		elif unit_faction == "universal":
			universal_units.append(unit_key)

	print("[Shop] _perform_refresh - faction_units: ", faction_units.size(), " universal_units: ", universal_units.size())
