...
func _on_totem_confirmed(totem_id: String):
	...
	# FIX-VIPER-SHOP-003: 确保 core_type 被设置
	# ActionDispatcher 应该已经设置了 core_type，但这里做一个保险
	if GameManager.core_type != totem_id:
		print("[CoreSelection] WARNING: GameManager.core_type (" + GameManager.core_type + ") 与 totem_id (" + totem_id + ") 不匹配，正在修复...")
		GameManager.core_type = totem_id
		print("[CoreSelection] 已设置 GameManager.core_type = " + totem_id)
...
func _create_cards():
	...
	if core_data.is_empty():
		print("Error: CORE_TYPES not found in Constants!")
		return
...
func _initialize_game_session():
	...
	if needs_shop_init:
		...
		print("[CoreSelection] GameManager.core_type: '", GameManager.core_type, "'")
		# DEBUG: 广播核心类型信息
		if AIManager:
			AIManager.broadcast_text("[CoreSelection] 当前阵营: '" + GameManager.core_type + "'")
		# 确保 BoardController 已初始化（总是使用最新的 session_data）
		if GameManager.session_data:
			print("[CoreSelection] 初始化 BoardController...")
			BoardController.initialize(GameManager.session_data)
		else:
			print("[CoreSelection] 错误: GameManager.session_data 为 null")
			if AIManager:
				AIManager.broadcast_text("[CoreSelection] 错误: session_data 为 null")
		# 刷新商店以获取初始单位
		var result = BoardController.refresh_shop()
		print("[CoreSelection] 商店刷新结果: ", result)
