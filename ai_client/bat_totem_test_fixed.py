#!/usr/bin/env python3
"""
蝙蝠图腾流派机制验证测试脚本 (BAT-TOTEM-001) - 修复版
使用 shop_helper 模块解决单位购买意图与实际不符问题
...
import aiohttp
from ai_client.shop_helper import ShopHelper


class BatTotemTesterFixed:
    """蝙蝠图腾流派测试器 - 修复版"""
    ...
    def __init__(self, http_port=8080):
        ...
        self.log_file = log_dir / f"ai_session_bat_totem_fixed_{timestamp}.log"
    ...
    async def deploy_unit(self, bench_pos: int, grid_pos: tuple):
        """部署单位从暂存区到战场"""
    ...
    async def step_buy_mosquito(self):
        """步骤2: 购买蚊子 - 使用shop_helper确保购买正确单位"""
        self.log("=" * 60, "SYSTEM")
        self.log("步骤2: 购买蚊子 - 验证吸食机制", "SYSTEM")
        self.log("=" * 60, "SYSTEM")

        # 使用shop_helper购买蚊子
        result = await self.shop_helper.buy_unit("mosquito", max_refresh=10)

        if result["success"] and result["unit_key"] == "mosquito":
            self.log(f"✅ 成功购买蚊子 (索引 {result['shop_index']}, 刷新 {result['refresh_count']} 次)", "VALIDATION")
        else:
            self.log(f"⚠️ 购买结果: {result}", "WARNING")

        # 部署蚊子
        await self.deploy_unit(0, (1, 0))
    ...
    async def step_buy_blood_mage(self):
        ...
        result = await self.shop_helper.buy_unit("blood_mage", max_refresh=10)

        if result["success"] and result["unit_key"] == "blood_mage":
            self.log(f"✅ 成功购买血法师 (索引 {result['shop_index']}, 刷新 {result['refresh_count']} 次)", "VALIDATION")
        else:
            self.log(f"⚠️ 购买结果: {result}", "WARNING")
    ...
    async def step_buy_life_chain(self):
        ...
        result = await self.shop_helper.buy_unit("life_chain", max_refresh=10)

        if result["success"] and result["unit_key"] == "life_chain":
            self.log(f"✅ 成功购买生命链条 (索引 {result['shop_index']}, 刷新 {result['refresh_count']} 次)", "VALIDATION")
        else:
            self.log(f"⚠️ 购买结果: {result}", "WARNING")

        await self.deploy_unit(0, (0, 1))
        await self.poll_observations(2.0)
    ...
    async def step_buy_blood_ritualist(self):
        ...
        result = await self.shop_helper.buy_unit("blood_ritualist", max_refresh=10)

        if result["success"] and result["unit_key"] == "blood_ritualist":
            self.log(f"✅ 成功购买血祭术士 (索引 {result['shop_index']}, 刷新 {result['refresh_count']} 次)", "VALIDATION")
        else:
            self.log(f"⚠️ 购买结果: {result}", "WARNING")
    ...
    async def generate_report(self):
        ...
        report_lines = [
            "\n" + "=" * 60,
            "蝙蝠图腾流派测试报告 (BAT-TOTEM-001) - 修复版",
            "=" * 60,
            f"测试时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            f"日志文件: {self.log_file}",
            "",
            "修复内容:",
            "  - 使用 shop_helper 模块确保购买意图与实际一致",
            "  - 自动查找目标单位索引",
            "  - 支持自动刷新商店直到找到目标单位",
            "",
            "验证结果:",
        ...
        report_file = report_dir / "qa_report_bat_totem_001_fixed.md"
    ...
    async def run_full_test(self):
        """运行完整测试"""
        self.log("=" * 60, "SYSTEM")
        self.log("开始蝙蝠图腾流派测试 (BAT-TOTEM-001) - 修复版", "SYSTEM")
        self.log("=" * 60, "SYSTEM")
...
async def main():
    ...
    async with BatTotemTesterFixed(http_port) as tester:
