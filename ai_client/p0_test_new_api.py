...
def run_p0_test(task_id, unit_name, unit_id, totem_id, test_notes=""):
    """
    使用新作弊API执行P0测试
    """
    # 使用新的命名规范
    task_name = task_id.lower().replace('-', '_')
    log_file = f"logs/ai_session_{task_name}_newapi_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    report_file = f"docs/player_reports/qa_report_{task_name}_newapi.md"

    log("=" * 70, log_file)
    log(f"【任务#15】P0测试 - {task_id} - {unit_name}", log_file)
    log(f"测试备注: {test_notes}", log_file)
    log("=" * 70, log_file)

    # 步骤1: 重置游戏
    log("\n[步骤1] 调用 reset_game() 重置游戏到初始状态", log_file)
    result = send_actions([{"type": "reset_game"}])
    log(f"[API响应] {result}", log_file)
    observations = wait_for_observations(3, log_file)
    for obs in observations:
        log(f"[OBS] {obs}", log_file)

    # 步骤2: 选择图腾
    log(f"\n[步骤2] 选择图腾 ({totem_id})", log_file)
    result = send_actions([{"type": "select_totem", "totem_id": totem_id}])
    log(f"[API响应] {result}", log_file)
    observations = wait_for_observations(3, log_file)
    for obs in observations:
        log(f"[OBS] {obs}", log_file)
    ...
    # 步骤3: 启用上帝模式
    log("\n[步骤3] 调用 set_core_hp(99999) 启用上帝模式保护核心", log_file)
    result = send_actions([{"type": "set_core_hp", "hp": 99999}])
    log(f"[API响应] {result}", log_file)
    wait_for_observations(2, log_file)

    # 步骤4: 使用spawn_unit生成目标单位（新API）
    log(f"\n[步骤4] 调用 spawn_unit() 生成目标单位: {unit_id}", log_file)
    spawn_action = {
        "type": "spawn_unit",
        "unit_id": unit_id,
        "grid_pos": {"x": 1, "y": 0}
    }
    log(f"[API请求] {spawn_action}", log_file)
    result = send_actions([spawn_action])
    log(f"[API响应] {result}", log_file)
    observations = wait_for_observations(3, log_file)
    for obs in observations:
        log(f"[OBS] {obs}", log_file)
    ...
    # 步骤6: 运行多波次战斗
    log("\n[步骤6] 运行战斗波次观察机制", log_file)
    all_logs = []

    for wave in range(1, 6):  # 运行5个波次
...
- **测试日志**: {log_file}
- **使用API**: reset_game() + spawn_unit() + set_core_hp()
...
测试执行者: AI Player (使用新作弊API - 任务#15)
"""
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python3 p0_test_new_api.py <task_id> <unit_name> <unit_id> <totem_id> [test_notes]")
        print("Example: python3 p0_test_new_api.py BAT-TOTEM-001 '蝙蝠图腾' bat bat_totem '流血攻击机制测试'")
        sys.exit(1)
    ...
    run_p0_test(task_id, unit_name, unit_id, totem_id, test_notes)
