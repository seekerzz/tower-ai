...
class ViperTester:
    ...
    async def __aenter__(self):
        ...
        self.log_file = Path(f"logs/ai_session_viper_totem_{timestamp}.log")
    ...
    async def run_test(self):
        self.log("=" * 60, "SYSTEM")
        self.log("开始毒蛇图腾流派测试 (TOTEM-VIPER-001)", "SYSTEM")
        self.log("=" * 60, "SYSTEM")
        ...
        # 选择毒蛇图腾
        self.log("选择毒蛇图腾...", "ACTION")
        await self.send_actions([{"type": "select_totem", "totem_id": "viper_totem"}])
        ...
        # 检查毒液相关日志
        for o in obs:
            if "毒" in o or "poison" in o.lower():
                self.log("✅ 发现毒液机制日志", "VALIDATION")
            if "中毒" in o:
                self.log("✅ 发现中毒机制日志", "VALIDATION")
...
async def main():
    ...
    async with ViperTester(http_port) as tester:
