...
**skip_to_wave**: 跳转到指定波次（调试用）。需提供 `wave` 字段，值为目标波次号（整数，如 6, 12, 18, 24）。

**debug_skip_to_secondary_totem**: 调试功能 - 直接跳转到次级图腾选择阶段。无需额外参数。使用后会模拟击败第1个Boss后的状态，弹出次级图腾选择界面。

**select_secondary_totem**: 选择次级图腾。需提供 `totem_id` 字段，可选值：`wolf_totem`, `cow_totem`, `bat_totem`, `viper_totem`, `butterfly_totem`, `eagle_totem`。注意：不能与主图腾相同。

**use_skill**: 使用单位技能。需提供 `grid_pos` 字段 {"x": int, "y": int}。

**reset_game**: 【作弊API】重置游戏到初始状态。无需参数。使用后会清空所有单位、重置波次为1、恢复核心满血、重置金币和法力。用于QA测试前重置环境。

**spawn_unit**: 【作弊API】直接生成指定单位。需提供 `unit_id` 字段（如 "iron_turtle", "hedgehog", "mosquito" 等），可选 `grid_pos` 字段 {"x": int, "y": int} 指定位置。如果不指定位置，会自动寻找第一个空位。用于绕过商店随机性直接获取测试目标单位。

示例：
```json
{"type": "reset_game"}
{"type": "spawn_unit", "unit_id": "iron_turtle"}
{"type": "spawn_unit", "unit_id": "hedgehog", "grid_pos": {"x": 1, "y": 1}}
```
