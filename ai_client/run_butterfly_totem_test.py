#!/usr/bin/env python3
"""
蝴蝶图腾流派测试启动脚本 (BUTTERFLY-TOTEM-001)
集成AI客户端启动和测试执行
"""
...
# 导入测试器
from butterfly_totem_test_001 import ButterflyTotemTester


class ButterflyTotemTestRunner:
    """蝴蝶图腾测试运行器"""
    ...
    def __init__(self, http_port=8080):
        ...
        self.log_file = log_dir / f"ai_client_butterfly_totem_{timestamp}.log"
    ...
    async def run_test(self):
        """运行测试"""
        self.log("=" * 60, "SYSTEM")
        self.log("启动BUTTERFLY-TOTEM-001测试", "SYSTEM")
        self.log("=" * 60, "SYSTEM")
        ...
        try:
            # 运行测试
            async with ButterflyTotemTester(self.http_port) as tester:
...
async def main():
    ...
    runner = ButterflyTotemTestRunner(http_port)
