#!/usr/bin/env python3
"""
EAGLE-VERIFY-003: Eagle修复重新验证测试 (CRITICAL-001修复后)
...
class EagleVerify003Tester:
    ...
    def __init__(self, http_port=8002):
    ...
    def start_ai_client(self):
        self.log(f"使用已运行的AI客户端 (端口{self.http_port})...", "SYSTEM")
        time.sleep(2)
    ...
    async def step_start_battle(self):
        ...
        # 生成孔雀和秃鹫放置在敌人路径上（地图四个角落）
        peacock_positions = [(2, 2), (-2, 2), (2, -2), (-2, -2)]
        vulture_positions = [(3, 1), (-3, 1), (1, 3), (-1, 3)]
            ...
            for o in obs:
                if "游戏结束" in o or "game over" in o.lower():
                    ...
                    if game_over_count >= 2:
                        return False
                # 检测孔雀易伤debuff日志
                if "PEACOCK_VULNERABLE" in o:
                    self.validation["peacock_vulnerable_log"] = True
                    self.log(f"✅ 检测到孔雀易伤debuff日志: {o}", "DETECTION")
                # 检测秃鹫死亡回响日志
                if "VULTURE_ECHO" in o:
                    self.validation["vulture_echo_log"] = True
                    self.log(f"✅ 检测到秃鹫死亡回响日志: {o}", "DETECTION")
                # 检测单位攻击日志
                if "单位攻击" in o or "UNIT ATTACK" in o or "攻击" in o:
                    self.validation["units_attacking"] = True
                # 检测[UNIT DEBUG]日志
                if "UNIT DEBUG" in o:
                    self.validation["process_called"] = True
    ...
    async def run_test(self) -> bool:
        self.log("=" * 70, "SYSTEM")
        self.log("EAGLE-VERIFY-003: Eagle修复重新验证 (CRITICAL-001修复后)", "SYSTEM")
        self.log("=" * 70, "SYSTEM")
