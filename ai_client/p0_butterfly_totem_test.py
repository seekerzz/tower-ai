#!/usr/bin/env python3
"""
P0测试: 蝴蝶图腾流派机制验证 (BUTTERFLY-TOTEM-001)
使用正常游戏流程（购买单位）进行测试
"""
...
LOG_FILE = f"{LOG_DIR}/ai_session_butterfly_totem_{session_time}.log"
REPORT_FILE = f"{REPORT_DIR}/qa_report_butterfly_totem_001_p0.md"
...
def test_butterfly_totem():
    log("=" * 60)
    log("P0测试: 蝴蝶图腾流派机制验证 (BUTTERFLY-TOTEM-001)")
    ...
    # 1. 选择蝴蝶图腾
    log("\n[步骤1] 选择蝴蝶图腾...")
    send_action({"type": "select_totem", "totem_id": "butterfly_totem"})
    time.sleep(2)
    logs = wait_for_logs(3)

    for entry in logs:
        if "butterfly_totem" in entry and "图腾选择" in entry:
    ...
    for entry in logs:
        if "购买" in entry or "butterfly" in entry.lower():
            log(f"[购买] {entry}")
    ...
    # 蝴蝶图腾关键词
    butterfly_keywords = ["蝴蝶", "图腾", "法球", "法力", "orb", "mana", "TOTEM", "BUTTERFLY"]
    for entry in logs:
        for kw in butterfly_keywords:
            if kw in entry:
                log(f"[战斗] {entry}")
                results["combat_logs"] = True
                if "法球" in entry or "orb" in entry.lower():
                    results["orb_mechanism"] = True
                break
...
def generate_report(results, logs):
    ...
    report = f"""# P0测试报告: 蝴蝶图腾流派机制验证 (BUTTERFLY-TOTEM-001)

## 测试信息
- **任务ID**: BUTTERFLY-TOTEM-001
...
| 验证项 | 状态 | 说明 |
|--------|------|------|
| 图腾选择 | {'通过' if results['totem_selection'] else '未通过'} | 蝴蝶图腾选择 |
| 商店刷新 | {'通过' if results['shop_refresh'] else '未通过'} | 商店内容刷新 |
| 单位购买 | {'通过' if results['unit_purchase'] else '未通过'} | 蝴蝶单位购买 |
| 波次启动 | {'通过' if results['wave_start'] else '未通过'} | 战斗波次开始 |
| 战斗日志 | {'通过' if results['combat_logs'] else '未通过'} | 战斗事件记录 |
| 法球机制 | {'通过' if results['orb_mechanism'] else '未通过'} | 法球生成与效果 |

## 关键日志摘录
"""

    bf_logs = [l for l in logs if any(k in l for k in ["蝴蝶", "法球", "法力", "butterfly", "orb", "mana"])]
    for i, log_entry in enumerate(bf_logs[:10], 1):
        report += f"\n{i}. {log_entry[:200]}"
...
### 蝴蝶法力光辉
- [ ] 消耗50法力: 每次攻击消耗50法力
- [ ] 附加100%伤害: 伤害附加消耗法力的100%

## 问题与限制
1. spawn_unit作弊API当前有bug，使用正常商店购买流程
2. 需要更多日志埋点来验证具体机制数值
3. 建议添加[ORB_GENERATE]、[MANA_RESTORE]等专用日志标记

## 附件
- 完整日志: `{LOG_FILE}`
- 测试脚本: `ai_client/p0_butterfly_totem_test.py`
...
def main():
    try:
        results, logs = test_butterfly_totem()
